...
export default class Popup extends Overlay {
  ...
  show(coord, html, title = 'Info', callback = null) {
    ...
    const header = document.getElementById('sc-popup-header');
    ...
    header.addEventListener('mousedown', (evt) => {
      ...
      const isCloser = evt.target.classList.contains('ol-popup-closer');
      ...
      let that = this;
      function move(evt) {
        let coord = window.map.getEventCoordinate(evt);
        let pixel = window.map.getPixelFromCoordinate(coord);
        let width =
          getWidth(window.map.getView().getProjection().getExtent()) /
          window.map.getView().getResolution();
        let pixelX = ((pixel[0] % width) + width) % width;
        let pixelY = pixel[1];
        let popupElem = document.getElementsByClassName('ol-popup')[0];
        let popupHeight = popupElem.offsetHeight;
        let popupWidth = popupElem.offsetWidth;

        // THIS SNAPS TO CENTER OF POPUP X
        let calc = pixelX - popupWidth / 4;
        let pixel2 = [calc, pixelY + popupHeight];
        let point = window.map.getCoordinateFromPixel(pixel2);
